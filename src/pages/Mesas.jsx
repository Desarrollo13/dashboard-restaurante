...
function ModalReserva({ mesas, onGuardar, onCerrar }) {
  const [form, setForm] = useState({
    mesa: '', cliente_nombre: '', cliente_telefono: '',
    fecha_hora: '', personas: 2, notas: '',
  })
  const [loading, setLoading] = useState(false)
  const [error,   setError]   = useState('')
      ...
      <div className="bg-gray-800 rounded-2xl p-6 w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <select name="mesa" value={form.mesa} onChange={handleChange} required
              className="w-full bg-gray-700 text-white rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input name="fecha_hora" value={form.fecha_hora}
            ...
            <div>
              ...
              <input name="personas" value={form.personas}
                onChange={handleChange} required type="number" min={1} max={20}
                className="w-full bg-gray-700 text-white rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500"/>
            </div>
          </div>
          <div>
...
export default function Mesas() {
  ...
  const [modalMesa,    setModalMesa]    = useState(false)  // ← acá, no en ModalMesa
  ...
  const handleConfirmar = async id => {
    try {
      ...
      fetchData(true)
    } catch { toast.error('Error al confirmar la reserva') }
  }
  ...
  const handleCompletar = async id => {
    try {
      ...
      fetchData(true)
    } catch { toast.error('Error al completar la reserva') }
  }
  ...
  return (
    <Layout>
              ...
              {reservas.map(r => (
                <div key={r.id} className="bg-gray-800 rounded-2xl p-5">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm mb-4">
                    <div><span className="text-gray-400">Mesa </span><span className="text-white font-medium">#{r.mesa_numero}</span></div>
                    <div><span className="text-gray-400">Personas </span><span className="text-white font-medium">{r.personas} 👤</span></div>
                    <div>
                    ...
                    <div><span className="text-gray-400">Creada por </span><span className="text-white font-medium">{r.creada_por_nombre || '—'}</span></div>
                  </div>
                  {r.notas && <p className="text-gray-500 text-sm mb-3 italic">📝 {r.notas}</p>}
                  <div className="flex gap-2 border-t border-gray-700 pt-3">
                    ...
                    {(r.estado === 'cancelada' || r.estado === 'completada') && (
                      <span className="text-gray-600 text-xs py-1.5">Sin acciones disponibles</span>
                    )}
                  </div>
                </div>
              ))}
      ...
      {/* ← Modales al final, dentro del return pero fuera del loading */}
      {modalMesa && (
        <ModalMesa
